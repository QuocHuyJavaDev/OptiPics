...
def run(cmd):
    process = subprocess.Popen(
        ...
        shell=True,
        encoding='utf-8',
        #thay the cac ky tu khong hop le
        errors='replace'
    )
    ...
    return process
# Lấy thời lượng (tg kết thúc của video) của tệp video từ đường dẫn path
def get_video_stream(path):
    try:
        ...
        if result.returncode == 0:
            #trip: loai bo khoang trang dư ra tu 2 ben
            return result.stdout.strip()
        else:
            return 0
    #In ra ngoai le neu co
    except Exception as err:
        print(str(err.stderr, encoding='utf8'))
        return 0

def cut_video(video_name, start_time=0, end_time=None):
    # Neu endtime chưa dddc chỉ định, thì gán thời lượng đã lấy ở trên cho endtime
    if not end_time:
